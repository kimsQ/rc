...
!(function ($) {
      'use strict';
      var Utility = function Utility(elem, options) {
            this.elem = elem;
            this.$elem = $(elem);
      ...
      Utility.prototype.setdataVal = function (component, dataAttr) {
            $.each(dataAttr, function (key, val) {
                  ...
                  if (valArr.length == 2) {
                        ...
                        if (valType == 'bg') $(target).css('background-image', 'url(' + val + ')');else if (valType == 'img') $(target).attr('src', val);else if (valType == 'inputText') $(target).val(val);else if (valType == 'html') $(target).html(val);
                  } else {
                        $(target).text(val);
                  }
            });
      ...
      Utility.prototype.resetHistoryObject = function (objType, objTarget) {
            ...
            if (objType == 'page') {
                  ...
                  $(object).removeClass('active');
                  if (objType != 'fbutton' && objType != 'drawer') setTimeout(function () {
                        $(object).hide();
                  }, ctime);
                  if (backdrop) $(bcontainer).find('.backdrop').remove();
                  if (objType == 'popover') $(object).popover('historyHide');else if (objType == 'sheet') $(object).sheet('historyHide');else if (objType == 'popup') $(object).popup('historyHide');else if (objType == 'fbutton') $(object).fbutton('historyHide');else if (objType == 'drawer') $(object).drawer('historyHide');
            }
