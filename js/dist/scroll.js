...
+(function ($) {
      ...
      var Scroll = function Scroll(element, options) {
            this.options = $.extend({}, Scroll.DEFAULTS, options);

            this.$target = $(this.options.target).on('scroll.rc.scroll.data-api', $.proxy(this.checkPosition, this)).on('tap.rc.scroll.data-api', $.proxy(this.checkPositionWithEventLoop, this));
      ...
      Scroll.prototype.checkPosition = function () {
            ...
            if (this.type == 'affix') {
                  if (this.affixed != affix) {
                        ...
                        this.$element.removeClass(Scroll.Affix_RESET).addClass(affixType).trigger(affixType.replace('affix', 'affixed') + '.rc.scroll');
                  }
                  ...
                  if (state == true) {
                        if (nowScrollTop < this.defaultHeight) scrollEvent = $.Event('default.rc.scroll');else {
                              if (nowScrollTop > lastScrollTop) scrollEvent = $.Event('down.rc.scroll');else scrollEvent = $.Event('up.rc.scroll');
                        }
